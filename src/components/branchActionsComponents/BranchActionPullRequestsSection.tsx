import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from "@/components/ui/accordion";
import { Button } from "@/components/ui/button";
import {
  CheckCircle,
  CircleEllipsis,
  SquircleDashed,
  XCircle,
} from "lucide-react";

import {
  HoverCard,
  HoverCardContent,
  HoverCardTrigger,
} from "@/components/ui/hover-card";

import { useEffect } from "react";
import { FaCopy } from "react-icons/fa6";
import { useDispatch } from "react-redux";
import { toast } from "sonner";
import { Separator } from "../ui/separator";

import type { Repository } from "@/redux/slices/createBranchesAndPrsSlices/branchActionsDataSlice";
import { handleBranchActionFetchPullRequestBuilds } from "./branchActions/handleBranchActionFetchPullRequestBuilds";
import { type AppDispatch } from "@/redux/store";

interface PRLinksViewerProps {
  projectName: string;
  repositoriesWithPRLinks: Repository[];
}

const POLL_INTERVAL_MS = 10000;

const BranchActionPullRequestsSection: React.FC<PRLinksViewerProps> = ({
  projectName,
  repositoriesWithPRLinks,
}) => {
  const dispatch = useDispatch<AppDispatch>();

  const copyAllPRs = () => {
    const textToCopy = repositoriesWithPRLinks
      .map((repo) => `${repo.name}: ${repo.pullRequestLink}`)
      .join("\n");

    if (!textToCopy) {
      toast.warning("No pull request links to copy.");
      return;
    }

    navigator.clipboard.writeText(textToCopy).then(() => {
      toast.success("Copied pull request links to clipboard.");
    });
  };

  const fetchBuilds = () => {
    void handleBranchActionFetchPullRequestBuilds(
      projectName,
      repositoriesWithPRLinks,
      dispatch
    );
  };

  useEffect(() => {
    fetchBuilds();
    const intervalId = setInterval(fetchBuilds, POLL_INTERVAL_MS);
    return () => clearInterval(intervalId);
  }, []);

  return (
    <Accordion
      type="single"
      collapsible
      className="border w-full rounded-lg mt-2"
    >
      <AccordionItem
        value="pull-request-links"
        className="px-6 bg-card rounded-lg"
      >
        <AccordionTrigger className="cursor-pointer font-bold">
          Pull Request Links
        </AccordionTrigger>

        <AccordionContent className="flex flex-col w-full gap-4 text-sm">
          <Separator className="my-2" />

          {repositoriesWithPRLinks.length > 0 ? (
            <>
              <Button
                size="sm"
                className="self-end text-xs"
                variant="outline"
                onClick={copyAllPRs}
              >
                <FaCopy className="mr-2" /> Copy All
              </Button>

              <div className="grid grid-cols-[repeat(auto-fit,_minmax(500px,_1fr))] gap-3 pb-4">
                {repositoriesWithPRLinks.map((repo: Repository) => (
                  <div
                    key={repo.name}
                    className="flex relative flex-col gap-1 p-4 border rounded-md"
                  >
                    <div className="flex flex-col gap-2">
                      <span className="font-medium">{repo.name}:</span>

                      <a
                        href={repo.pullRequestLink}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-primary block max-w-[95%] truncate"
                        title={repo.pullRequestLink}
                      >
                        {repo.pullRequestLink}
                      </a>

                      {repo.build && (
                        <div className="absolute top-4 right-4">
                          <HoverCard>
                            <HoverCardTrigger asChild>
                              <div className="inline-flex items-center space-x-2 cursor-pointer">
                                {repo.build.successful > 0 && (
                                  <div className="inline-flex items-center space-x-1 text-[#4BC78E]">
                                    <CheckCircle className="h-4 w-4" />
                                    <span>{repo.build.successful}</span>
                                  </div>
                                )}
                                {repo.build.inProgress > 0 && (
                                  <div className="inline-flex items-center space-x-1 text-yellow-500 dark:text-yellow-400">
                                    <SquircleDashed className="h-4 w-4 animate-spin" />
                                    <span>{repo.build.inProgress}</span>
                                  </div>
                                )}
                                {repo.build.failed > 0 && (
                                  <div className="inline-flex items-center space-x-1 text-destructive">
                                    <XCircle className="h-4 w-4" />
                                    <span>{repo.build.failed}</span>
                                  </div>
                                )}
                                <span className="ml-2 text-xs">
                                  <CircleEllipsis className="h-4 w-4" />
                                </span>
                              </div>
                            </HoverCardTrigger>

                            <HoverCardContent className="text-xs space-y-1 w-fit">
                              <div>
                                <span className="inline-block w-2 h-2 rounded-full bg-[#4BC78E] mr-2"></span>
                                Successful:
                                <span className="ml-2 font-semibold">
                                  {repo.build.successful}
                                </span>
                              </div>
                              <div>
                                <span className="inline-block w-2 h-2 rounded-full bg-destructive mr-2"></span>
                                Failed:
                                <span className="ml-2 font-semibold">
                                  {repo.build.failed}
                                </span>
                              </div>
                              <div>
                                <span className="inline-block w-2 h-2 rounded-full bg-yellow-500 dark:bg-yellow-400 mr-2"></span>
                                In Progress:
                                <span className="ml-2 font-semibold">
                                  {repo.build.inProgress}
                                </span>
                              </div>
                              <div>
                                <span className="inline-block w-2 h-2 rounded-full bg-gray-500 mr-2"></span>
                                Cancelled:
                                <span className="ml-2 font-semibold">
                                  {repo.build.cancelled}
                                </span>
                              </div>
                              <div>
                                <span className="inline-block w-2 h-2 rounded-full bg-primary mr-2"></span>
                                Unknown:
                                <span className="ml-2 font-semibold">
                                  {repo.build.unknown}
                                </span>
                              </div>
                            </HoverCardContent>
                          </HoverCard>
                        </div>
                      )}
                    </div>
                  </div>
                ))}
              </div>
            </>
          ) : (
            <p className="text-muted-foreground px-4 py-2">
              No pull request links available.
            </p>
          )}
        </AccordionContent>
      </AccordionItem>
    </Accordion>
  );
};

export default BranchActionPullRequestsSection;
