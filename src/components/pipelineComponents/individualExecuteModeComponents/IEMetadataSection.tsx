...
const IEMetadataSection: React.FC<Props> = ({ dependency, onChange }) => {
  const handleVersionChange = (val: string) => {
    onChange("selectedVersion", val);

    const depName = dependency.name;
    onChange("commitMessage", `Update ${depName} to ${val}`);
    onChange("pullRequestTitle", `Update ${depName} to ${val}`);
    onChange(
      "pullRequestDescription",
      `This pull request updates the dependency **${depName}** to version **${val}**.`
    );
  };
...
export default IEMetadataSection;
